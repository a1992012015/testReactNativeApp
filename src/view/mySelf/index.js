...
class BottomMenu extends PureComponent {
    render() {
        ...
        return (
            <TouchableOpacity
                onPress={() => {onClickChange(component)}}
                activeOpacity={.8}
    ...
    realNameJump = (page, memberInfo) =>{
        ...
        if(states === 0 || states === "0"){
            Alert.alert(
                ...
                [{
                    text: '确认',
                    onPress: () => this.props.navigation.navigate("realAuthentication", {member: memberInfo, infoAction: this.upState})
                }]
            );
        ...
        return (
            <View style={[styles.defaultView, {paddingBottom: p(100)}]}>
                ...
                <View style={styles.header}>
                    {/*左侧铃铛消息组件*/}
                    <TouchableOpacity
                        onPress={() => {this.onClickChange("MyNews")}}
                        activeOpacity={.8}>
                        <Icon
                    ...
                    {/*右侧设置组件*/}
                    <TouchableOpacity
                        onPress={() => {this.onClickChange("MySetUp", this.state.memberInfo)}}>
                        <Icon
                            name="md-settings"
                            size={25}
                            color='#fff'
                ...
                >
                    ...
                    <View style={styles.blockSty}>
                        <View style={styles.topItemSty}>
                            <Text style={styles.itemTextSty}>账户：<Text style={{color:'#FFF'}}>{this.state.username}</Text></Text>
                        </View>
                    </View>
                    {/*充币和提币组件*/}
                    <View style={styles.blockSty}>
                        <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center'}}>
                            {/*充币组件*/}
                            <TouchableOpacity
                                onPress={() => {this.jumpPage("IntoCurrencyList")}}
                                activeOpacity={.8}
                            ...
                            {/*提币组件*/}
                            <TouchableOpacity
                                onPress={() => {this.jumpPage("TurnoutCurrencyList")}}
                                activeOpacity={.8}
                                style={[styles.midBlockSty, {borderLeftWidth: StyleSheet.hairlineWidth, borderLeftColor: '#41484F'}]}>
                                <Icon name="ios-undo"
                                      size={20}
                                      color='#fff'
                                      style={{paddingHorizontal:p(20)}}
