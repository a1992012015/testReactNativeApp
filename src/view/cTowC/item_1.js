...
export default class Item_1 extends PureComponent {
    ...
    appCreateTransaction = () => {
        ...
        const actions = {
            transactionType: 1,
            transactionPrice: this.state.buyMoney,
        ...
        if (this.state.loadData) {
            return (
                <View style={{flex: 1, marginBottom: config.api.isTabView ? p(100) : 0}}>
                    ...
                    {/*存疑？ => 完成订单的弹出窗*/}
                    <BuySellModal isOpen={this.state.isOpen} setItemText={this.setItemText} isType="buy" {...this.props}
                                  buySellData={this.state.buySellData} isGoBack={false}/>
                </View>
            )
        } else {
            return (
